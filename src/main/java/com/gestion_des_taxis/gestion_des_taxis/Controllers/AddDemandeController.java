...
public class AddDemandeController implements Initializable {
 ...
 /*try {
      ...
      throw new RuntimeException(e);
        ...
        if (vtr.isEmpty() || client.isEmpty() || chauffeur.isEmpty()
            ...
            if (status.isEmpty()) {
                ...
                new Shake(cbstatus).play();
                errorMessage.setVisible(true);
                errorMessage.setText("status est obligatoire!");
                new BounceIn(errorMessage).play();
            } else {
                cbstatus.setStyle("-fx-border-color: white;");
            }
    ...
    public void fillStatus() {
        ...
        statusList.clear();
        statusList.add("En attente");
        statusList.add("Annulé");
        statusList.add("Accepté");
        statusList.add("Refusé");
    ...
    void fillVoitureMarque(ActionEvent event) {
        String value = cbvoiture.getValue();
        con = DataBaseConnection.GetConnection();
        try {
